import { db } from "@/db";
import { tokenTable } from "@/db/schemas/otp";
import { env } from "@/env";
...
export async function saveToken(target: string, otp: string, otpExpireAt: Date) {
    ...
    return db.insert(tokenTable).values({ target, token: hashedOtp, tokenExpireAt: otpExpireAtIso }).onConflictDoUpdate({
...
export async function verifyToken(token: string, target: string) {
    ...
    await db.delete(tokenTable).where(and(eq(tokenTable.target, target), eq(tokenTable.token, token)))
    return true;
}
...
export async function sendOfficialMailVerificationLink(officeEmail: string, token: string) {
    try {
        const verificationUrl = `${env.SERVER_URL}/long-form/verification/verify-office-email/${token}`
